      ...
      <div style={{
        ...
        <div style={{ padding: 24 }}>
              ...
              <>
                ...
                >
                  <span style={{ color: '#fff', fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <UploadSimple size={18} /> Rasmni almashtirish
                  </span>
                </div>
              </>
            ) : (
              <>
                <ImageSquare size={40} color={dragging ? '#059669' : C.muted} weight="duotone" />
                <div style={{ marginTop: 10, fontWeight: 600, color: C.muted, fontSize: 14 }}>
                  Rasm tanlash yoki bu yerga tashlang
                </div>
                <div style={{ marginTop: 4, fontSize: 12, color: C.muted }}>PNG, JPG, WEBP</div>
              </>
            )}
            <input
              ref={fileRef}
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input className="input-focus" type="text" value={name} onChange={e => setName(e.target.value)}
              placeholder="Masalan: Shokoladli tort" style={iStyle} />
          </div>

          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: C.muted, display: 'block', marginBottom: 6 }}>Narxi (so'm)</label>
            <input className="input-focus" type="number" value={price} onChange={e => setPrice(e.target.value)}
              placeholder="150000" style={iStyle} />
          </div>
          ...
          {err && (
            <div style={{ color: '#ef4444', fontSize: 13, marginBottom: 14, background: 'rgba(239,68,68,.08)', padding: '10px 14px', borderRadius: 10 }}>
              {err}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={onClose} style={{
              flex: 1, padding: '13px 0', borderRadius: 12, border: `1.5px solid ${C.border}`,
            ...
            <button onClick={handleSave} disabled={saving} style={{
              ...
              background: saving ? '#059669' : 'linear-gradient(135deg,#064e3b,#059669)',
...
/* ─── Order card ─────────────────────────────────────── */
function OrderCard({ order, C, onConfirm, onCancel, onChat, showSeller }) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        <div>
          ...
          {showSeller && order.seller_name && (
            <div style={{ fontSize: 12, color: '#059669', fontWeight: 600, marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
              <Storefront size={12} /> {order.seller_name}
            </div>
          )}
        </div>
        <span style={{
          fontSize: 12, fontWeight: 700, color: meta.color,
          background: meta.bg, padding: '4px 12px', borderRadius: 50,
        }}>
          {meta.label}
        </span>
      </div>
...
export default function SellerDashboardPage({ seller, onLogout, C, isDesktop }) {
  ...
  const otherOrders = orders.filter(o => o.seller_id !== seller?.id && ['confirmed','ready'].includes(o.status));
  ...
  const handleChat = order => {
    setChatData({ orderId: order.id, userId: order.user_id, sellerId: seller?.id });
    setShowChat(true);
  };
  ...
  const TABS = [
    ...
    { id: 'accepted', icon: <CheckCircle size={20} />, label: 'Qabul qilingan' },
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 740, margin: '0 auto', position: 'relative' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            {[
              { icon: <ClipboardText size={20} />, val: orders.length,   label: 'Jami buyurtma' },
        ...
        <div style={{ padding: '16px 16px 100px' }}>
          ...
          {tab === 'orders' && (
            loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '64px 20px' }}>
                ...
                <div style={{ fontWeight: 700, color: C.dark, marginTop: 14, marginBottom: 6 }}>Yangi buyurtmalar yo'q</div>
          ...
          {tab === 'accepted' && (
            loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '64px 20px' }}>
                <CheckCircle size={56} weight="duotone" color={C.muted} style={{ opacity: .35 }} />
                <div style={{ fontWeight: 700, color: C.dark, marginTop: 14, marginBottom: 6 }}>Qabul qilingan buyurtmalar yo'q</div>
            ...
            <div>
              ...
              {products.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                  ...
                  <div style={{ color: C.muted, fontSize: 13 }}>Yuqoridagi tugma orqali birinchi tortingizni qo'shing</div>
                </div>
              ) : (
                    ...
                    <div key={product.id} style={{
                      ...
                      <div style={{
                        ...
                        background: product.image
                          ...
                          : `linear-gradient(135deg, #064e3b22, #05966922)`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: 52,
                      }}>
                      ...
                      <button onClick={() => deleteProduct(product.id)} style={{
                        ...
                        border: 'none', background: 'rgba(220,38,38,.85)',
